...
const product = products.find((p) => p.slug === "cross-border-finance")!;
const otherProducts = products.filter((p) => p.slug !== "cross-border-finance");
const accent = product.color;
const heroImage = "/images/products/cross-border-hero.png";

const eligibilityIcons = [Globe, TrendingUp, FileCheck, Shield];
const statIcons = [Banknote, Clock, Percent, MapPin];

export default function CrossBorderFinancePage() {
  ...
  const handleBrochureSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/brochure", {
        ...
        body: JSON.stringify({ email, product: product.slug, brochureFile: product.brochureFile }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Something went wrong");
      setSuccess(true);
      toast({ title: "Brochure download started!", description: data.message || "Your download is starting." });
    ...
    <div>
              ...
              <SlideReveal direction="right" delay={0.2}>
                <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                  <motion.div style={{ y: parallaxY }}>
                    <Image
                      src={heroImage}
                      alt="Cross border trade and international finance"
                      width={600}
                      height={400}
        ...
        <section className="py-16 md:py-20 bg-[#F7F9FC]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <SectionReveal>
              ...
              <p className="text-[#718096] mb-10 ml-12 max-w-2xl">Key requirements to qualify for cross-border trade finance solutions.</p>
            </SectionReveal>
      ...
      <section id="products-section" className="py-16 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
            ...
            <p className="text-[#718096] mb-10 ml-12 max-w-2xl">End-to-end trade finance solutions covering every stage of the international trade cycle.</p>
          </SectionReveal>
      ...
      {product.processSteps && product.processSteps.length > 0 && (
        <ProcessInfographic
          ...
          accentColor={accent}
          subtitle="From trade assessment to funding and monitoring — our process ensures smooth cross-border transactions."
        />
      )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <SlideReveal direction="left">
                ...
                <p className="text-[#718096] mb-8 max-w-lg">Why exporters and importers choose Credora for their cross-border finance needs.</p>
              ...
              <SlideReveal direction="right" delay={0.2}>
                <div className="relative flex items-center justify-center">
                  <div className="w-80 h-80 sm:w-96 sm:h-96 lg:w-[32rem] lg:h-[32rem]">
                    <img src="/images/sections/cross-border-finance.png" alt="International cross-border trade with cargo ships at modern port" className="rounded-2xl shadow-xl w-full h-full object-cover" />
                  </div>
                </div>
              </SlideReveal>
            </div>
        ...
        <section className="py-16 md:py-20 bg-[#F7F9FC]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <SectionReveal>
              ...
              <p className="text-[#718096] mb-10 ml-12 max-w-2xl">Common questions about cross-border finance, documentation, and compliance.</p>
            </SectionReveal>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
            <div className="rounded-2xl border-2 p-8 sm:p-10 md:p-14 text-center relative overflow-hidden" style={{ borderColor: `${accent}30` }}>
              ...
              <p className="text-[#718096] mb-8 max-w-lg mx-auto">Get our detailed brochure with trade finance product details, documentation checklists, and compliance guidelines.</p>
              {success ? (
                <div className="flex flex-col items-center justify-center gap-2 text-[#87B73C]">
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-6 h-6" />
                    <span className="text-lg font-semibold">Download started! Check your browser downloads.</span>
                  </div>
                  <p className="text-sm text-[#718096]">If the download didn&apos;t start automatically, please disable your popup blocker and try again.</p>
                </div>
              ) : (
                <form onSubmit={handleBrochureSubmit} className="flex flex-col sm:flex-row items-center gap-3 max-w-md mx-auto">
                  ...
                  <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                    <Button type="submit" disabled={loading} className="h-12 px-7 text-white font-semibold text-sm uppercase tracking-wider rounded-md whitespace-nowrap" style={{ backgroundColor: accent }}>
                      {loading ? (
                        <span className="flex items-center gap-2"><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Preparing…</span>
                      ) : (
      ...
      <section className="py-16 md:py-20 relative overflow-hidden" style={{ backgroundColor: "#1C1D62" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <SectionReveal>
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-4">Power Your International Trade</h2>
              <p className="text-white/70 mb-0 text-base sm:text-lg">Whether you&apos;re exporting or importing, our trade finance specialists ensure smooth cross-border transactions with competitive terms.</p>
            </SectionReveal>
