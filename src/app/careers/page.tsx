...
const cultureValues = [
  { icon: Rocket, title: "Growth Mindset", desc: "We invest in learning. Every team member gets access to training, certifications, and mentorship to accelerate their career.", color: "#304AC0" },
  { icon: Heart, title: "Client First", desc: "Everything we do starts with the client. We celebrate when a business gets the funding it needs to thrive.", color: "#87B73C" },
  { icon: Users, title: "Collaborative Spirit", desc: "No silos here. Credit, sales, and operations work as one team to deliver results and learn from each other.", color: "#13277E" },
  { icon: Shield, title: "Integrity & Transparency", desc: "We operate with the highest ethical standards. Honest advice, clear communication, and zero hidden charges.", color: "#1C1D62" },
];

const perks = [
  { icon: IndianRupee, label: "Competitive Salary", desc: "Market-competitive pay with performance bonuses" },
  { icon: TrendingUp, label: "Career Growth", desc: "Clear growth paths with quarterly reviews" },
  { icon: BookOpen, label: "Learning Budget", desc: "Annual learning allowance for courses and certs" },
  { icon: Coffee, label: "Flexible Culture", desc: "Hybrid work options and supportive environment" },
  { icon: Heart, label: "Health Benefits", desc: "Medical insurance for you and your family" },
  { icon: Award, label: "Recognition", desc: "Monthly awards, team outings, and celebrations" },
];

const hiringSteps = [
  { title: "Apply", description: "Browse open positions and submit your application with your resume.", icon: Send, accent: "#304AC0" },
  { title: "Screen", description: "Our HR team reviews your profile and reaches out within 3 business days.", icon: FileSearch, accent: "#13277E" },
  { title: "Interview", description: "A role-specific technical/functional round followed by a culture-fit conversation.", icon: Users, accent: "#87B73C" },
  { title: "Offer", description: "Receive a competitive offer and join the Credora family!", icon: Sparkles, accent: "#304AC0" },
];
...
export default function CareersPage() {
  ...
  // Fetch positions from the API on mount
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!cancelled) {
          setOpenPositions(json.data ?? []);
        }
      } catch (err) {
        console.error("[Careers] failed to load positions:", err);
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Thank you for applying. Our team will review your profile and get back to you within 3 business days.",
      });
  ...
  return (
    ...
    >
      {/* ═══════════════════════════════════════
          PAGE HERO — Split layout with stats cards
          ═══════════════════════════════════════ */}
      <section className="bg-[#F0F4FF] py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
          <SectionReveal>
            <nav className="flex items-center gap-1.5 text-sm text-[#718096] mb-8" aria-label="Breadcrumb">
              <Link href="/" className="hover:text-[#304AC0] transition-colors duration-300">
                Home
              </Link>
              <ChevronRight className="w-3.5 h-3.5" />
              <span className="text-[#1C1D62] font-medium">Careers</span>
            </nav>
          </SectionReveal>

          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            {/* LEFT — Heading with briefcase watermark */}
            <SectionReveal direction="left">
              <div className="relative">
                {/* Decorative briefcase SVG watermark */}
                <svg viewBox="0 0 200 200" className="absolute -top-10 -left-10 w-48 h-48 opacity-[0.07] pointer-events-none" aria-hidden="true">
                  <rect x="30" y="70" width="140" height="100" rx="12" fill="#304AC0" />
                  <rect x="65" y="50" width="70" height="40" rx="8" fill="#304AC0" />
                  <rect x="55" y="100" width="90" height="8" rx="4" fill="white" opacity="0.5" />
                  <rect x="55" y="120" width="60" height="8" rx="4" fill="white" opacity="0.3" />
                </svg>

                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-semibold text-[#1C1D62] leading-tight relative z-10">
                  Build Your Future{" "}
                  <span className="relative inline-block">
                    <span className="text-[#304AC0]">in Fintech</span>
                    <motion.span
                      className="absolute -bottom-1 left-0 right-0 h-1.5 bg-[#304AC0]/20 rounded-full"
                      initial={{ scaleX: 0 }}
                      animate={{ scaleX: 1 }}
                ...
                <p className="mt-5 text-[#2D3748] leading-relaxed max-w-xl text-base sm:text-lg relative z-10">
                  Join Credora Fintech and help thousands of MSMEs access the funding
                  they need. We&apos;re a young, ambitious team solving real problems
                  in India&apos;s credit gap — and we want you on board.
                </p>
            ...
            <SectionReveal direction="right" delay={0.15}>
              <div className="relative flex flex-col items-center gap-5">
                {[
                  { value: 50, suffix: "+", label: "Team Members", icon: Users, color: "#304AC0", bg: "bg-[#F0F4FF]" },
                  { value: 1200, suffix: "+", label: "Clients Served", icon: Briefcase, color: "#87B73C", bg: "bg-[#F5F8EC]" },
                  { value: 70, suffix: "+", label: "Lender Partners", icon: Building2, color: "#304AC0", bg: "bg-[#F0F4FF]" },
                ].map((card, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    ...
                    >
                      <motion.div
                        className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${card.bg}`}
                      ...
                      <div>
                        <p className="text-2xl font-bold text-[#1C1D62]">
                          <AnimatedNumber value={card.value} />
                          <span className="text-base font-semibold text-[#718096]">{card.suffix}</span>
                        </p>
                        <p className="text-[#718096] text-sm mt-0.5">{card.label}</p>
                      </div>
                    </div>
                  </motion.div>
                ))}
      ...
      {/* ═══════════════════════════════════════
          WHY WORK HERE — Culture values cards
          ═══════════════════════════════════════ */}
      ...
      {/* ═══════════════════════════════════════
          PERKS & BENEFITS — 2x3 grid with accent
          ═══════════════════════════════════════ */}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <SectionReveal direction="left">
              ...
              <p className="text-[#2D3748] leading-relaxed mb-8">
                ...
                Beyond competitive compensation, we offer a range of benefits
                designed to help you grow personally and professionally while
              ...
              <StaggerContainer className="grid sm:grid-cols-2 gap-5" staggerDelay={0.1}>
                {perks.map((perk, i) => (
                  <StaggerItem key={i}>
                    ...
                    >
                      <div
                        className={`${i % 2 === 0 ? "bg-white" : "bg-[#F5F8EC]"} rounded-2xl p-5 h-full min-h-[120px] flex flex-col border border-[#E8ECF0] group transition-all duration-300`}
                        style={{ borderTopWidth: "4px", borderTopColor: i % 2 === 0 ? "#304AC0" : "#87B73C" }}
                      >
                        <motion.div
                          className="w-10 h-10 rounded-lg bg-white flex items-center justify-center mb-3 shadow-sm"
                          whileHover={{ rotate: 5, scale: 1.1 }}
                          transition={{ type: "spring", stiffness: 300, damping: 15 }}
                        >
                          <perk.icon className="w-5 h-5" style={{ color: i % 2 === 0 ? "#304AC0" : "#87B73C" }} />
                        </motion.div>
                        <p className="text-[#1C1D62] font-semibold text-sm mb-1">{perk.label}</p>
                        <p className="text-[#718096] text-xs leading-relaxed flex-1">{perk.desc}</p>
                      </div>
                    </motion.div>
                  </StaggerItem>
                ))}
      ...
      {/* ═══════════════════════════════════════
          OPEN POSITIONS — Filterable job cards
          ═══════════════════════════════════════ */}
      <section id="open-positions" className="py-16 md:py-20 bg-white scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
              ...
              {departments.map((dept) => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                    selectedDept === dept
                      ? "bg-[#304AC0] text-white shadow-md"
                      : "bg-[#F0F4FF] text-[#2D3748] hover:bg-[#E0E7FF]"
                  }`}
          ...
          {loadingPositions ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="bg-white rounded-2xl p-6 border border-[#E8ECF0] shadow-sm h-64 animate-pulse">
                  <div className="h-3 w-24 bg-[#F0F4FF] rounded mb-4" />
                  <div className="h-5 w-3/4 bg-[#F0F4FF] rounded mb-3" />
                  <div className="h-3 w-1/2 bg-[#F0F4FF] rounded mb-2" />
                  <div className="h-3 w-2/3 bg-[#F0F4FF] rounded mb-6" />
                  <div className="h-16 w-full bg-[#F0F4FF] rounded" />
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-16">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[#F0F4FF] flex items-center justify-center">
                <Briefcase className="w-8 h-8 text-[#304AC0]" />
              </div>
              <h3 className="text-lg font-semibold text-[#1C1D62] mb-2">No Open Positions Right Now</h3>
              <p className="text-[#718096] text-sm max-w-md mx-auto">
                We don&apos;t have any openings in this department at the moment.
                Please check back soon or submit a general application below —
            ...
            {filteredPositions.map((pos) => (
              <StaggerItem key={pos.id}>
                <motion.div
                  ...
                  transition={{ type: "spring", stiffness: 300, damping: 20 }}
                >
                  <div className="bg-white rounded-2xl p-6 border border-[#E8ECF0] shadow-sm h-full flex flex-col group transition-all duration-300 relative overflow-hidden">
                    {/* Colored top accent */}
                    <div className="absolute top-0 left-0 right-0 h-1.5" style={{ backgroundColor: pos.color }} />

                    {/* Department & Type badges */}
                    <div className="flex items-center gap-2 mt-2 mb-4">
                      <span className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full" style={{ backgroundColor: `${pos.color}12`, color: pos.color }}>
                        {pos.department}
                      </span>
                      <span className="text-[10px] font-medium text-[#718096] bg-[#F0F4FF] px-2.5 py-1 rounded-full">
                        {pos.type}
                      </span>
                    </div>
                    ...
                    {/* Meta info */}
                    <div className="flex flex-wrap gap-3 mb-3 text-xs text-[#718096]">
                      <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{pos.location}</span>
                      <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{pos.experience}</span>
                    </div>
                    <p className="text-[#2D3748] text-sm font-medium mb-3">{pos.salary}</p>

                    <p className="text-[#718096] text-sm leading-relaxed flex-1 mb-4">{pos.description}</p>

                    {/* Skills tags */}
                    <div className="flex flex-wrap gap-1.5 mb-5">
                      {pos.skills.map((skill, j) => (
                        <span key={j} className="text-[11px] text-[#304AC0] bg-[#F0F4FF] px-2 py-0.5 rounded-md">
                          {skill}
                        </span>
                      ))}
                    </div>
                    ...
                    <a href="#apply-form">
                      <Button
                        variant="outline"
                        className="w-full border-[#304AC0] text-[#304AC0] hover:bg-[#304AC0] hover:text-white text-sm font-medium rounded-lg group/btn transition-all duration-300"
                      >
                        Apply Now
                        <ArrowRight className="ml-1.5 w-3.5 h-3.5 group-hover/btn:translate-x-1 transition-transform duration-300" />
                      </Button>
                    </a>
                  </div>
                </motion.div>
                ...
                >
                  <div
                    className={`relative bg-white rounded-2xl border border-[#E8ECF0] shadow-sm p-6 sm:p-8 h-full min-h-[200px] flex flex-col text-center ${
                      i < hiringSteps.length - 1 ? "md:mr-6" : ""
                  ...
                  >
                    ...
                    <span className="inline-block text-xs font-bold uppercase tracking-widest mb-2" style={{ color: step.accent }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
            <div className="max-w-3xl mx-auto relative">
              <svg viewBox="0 0 80 80" className="absolute -top-6 -left-4 w-16 h-16 opacity-10 pointer-events-none" aria-hidden="true">
                <text x="0" y="65" fontSize="80" fontFamily="Georgia, serif" fill="#304AC0">&ldquo;</text>
              </svg>
                ...
                <div className="flex flex-col md:flex-row items-start gap-6">
                  ...
                  <div>
                    <blockquote className="text-lg sm:text-xl font-medium text-[#1C1D62] leading-relaxed italic">
                      Credora gave me the opportunity to grow from a fresher to a
                      credit analyst handling complex MSME profiles. The learning
                      never stops, and the team genuinely supports your development.
                    </blockquote>
                    <div className="mt-5 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#304AC0] flex items-center justify-center text-white font-bold text-sm">
                        AK
                      </div>
                      <div>
                        <p className="text-[#1C1D62] font-semibold text-sm">Anitha Krishnan</p>
                        <p className="text-[#718096] text-xs">Credit Analyst — Credora Fintech</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* ═══════════════════════════════════════
          WHY CREDORA — Dark card with progress bars
          ═══════════════════════════════════════ */}
      <section className="py-16 md:py-20 bg-[#F0F4FF]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
            <div className="bg-[#1C1D62] rounded-2xl p-8 sm:p-10 md:p-12 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <StaggerContainer className="flex flex-col gap-5" staggerDelay={0.1}>
                  {[
                    { text: "Real impact — help businesses access the funding they need to grow", progress: 95 },
                    { text: "Flat hierarchy — your ideas are heard and implemented from day one", progress: 90 },
                    { text: "Exposure to 70+ lenders and the full spectrum of financial products", progress: 100 },
                    { text: "Fast-growing fintech with clear career progression paths", progress: 88 },
                  ].map((item, i) => (
                    <StaggerItem key={i}>
                      <motion.div
                        ...
                        transition={{ type: "spring", stiffness: 300, damping: 20 }}
                      >
                        <p className="text-white/90 leading-relaxed text-base sm:w-80 flex-shrink-0">
                          {item.text}
                        </p>
                        <div className="flex-1 h-3 rounded-full bg-white/10 overflow-hidden">
                          <motion.div
                            className="h-full rounded-full"
                            style={{ backgroundColor: i % 2 === 0 ? "#87B73C" : "#304AC0" }}
                            initial={{ width: 0 }}
      ...
      {/* ═══════════════════════════════════════
          APPLICATION FORM
          ═══════════════════════════════════════ */}
      <section id="apply-form" className="py-16 md:py-20 bg-white scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <SectionReveal>
            <div className="max-w-3xl mx-auto">
              <div className="text-center mb-10">
                ...
                <p className="text-[#718096] text-sm sm:text-base max-w-lg mx-auto">
                  ...
                  profile and reach out within 3 business days if there&apos;s a fit.
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 sm:p-8 md:p-10 border border-[#E8ECF0] shadow-lg">
                <form onSubmit={handleSubmit} noValidate className="space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="position" className="text-[#2D3748] text-sm font-medium">
                      ...
                      >
                        <SelectTrigger className={`w-full h-11 rounded-lg border-[#E8ECF0] focus:border-[#304AC0] focus:ring-[#304AC0]/20 ${formErrors.position ? "border-red-400" : ""}`}>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="experience" className="text-[#2D3748] text-sm font-medium">
                        Total Experience <span className="text-[#718096] font-normal">(optional)</span>
                      </Label>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="message" className="text-[#2D3748] text-sm font-medium">
                      Cover Note / Additional Details <span className="text-[#718096] font-normal">(optional)</span>
                    </Label>
                    <Textarea
                      id="message"
                      placeholder="Tell us about yourself, your relevant experience, and why you'd like to join Credora..."
                      rows={4}
                      value={formData.message}
                      onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                      className="rounded-lg border-[#E8ECF0] focus:border-[#304AC0] focus:ring-[#304AC0]/20 resize-none"
                    />
                  </div>
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          {Array.from({ length: 24 }).map((_, i) => (
            <motion.span
              ...
              transition={{ duration: 2 + (i % 3), delay: i * 0.15, repeat: Infinity, ease: "easeInOut" }}
            />
          ))}
        </div>
