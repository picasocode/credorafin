...
const processTimeline = [
  { step: 1, icon: Users, title: "Customer Requirement Analysis", desc: "Understand your specific funding need, amount, purpose, timeline, and business context" },
  ...
  { step: 3, icon: Landmark, title: "Multi-Lender Presentation", desc: "Present your case to multiple lenders simultaneously with a single Credit enquiry" },
  { step: 4, icon: BarChart3, title: "Offer Comparison", desc: "Compare offers from multiple lenders and identify the best pricing and terms for your business" },
  { step: 5, icon: TrendingUp, title: "Sanction & Disbursal", desc: "Continuous follow-ups until sanction is received and funds are disbursed to your account" },
];

const processIcons = [Users, FileCheck, Landmark, BarChart3, TrendingUp];

const lenders = [
  { type: "PSU Bank", rate: "9.5%" },
  { type: "Private Bank", rate: "10.2%" },
  { type: "NBFC", rate: "11.0%" },
  { type: "Private Bank", rate: "9.8%" },
  { type: "Private Bank", rate: "10.5%" },
  { type: "NBFC", rate: "10.8%" },
];
...
export default function FundRaisingPage() {
  ...
  return (
    <div className="overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <FadeIn>
            <div className="text-center mb-14">
              ...
              <p className="text-[#718096] max-w-2xl mx-auto">Your application is structured once and presented to multiple lenders simultaneously — giving you access to the best terms with a single Credit enquiry.</p>
            </div>
          </FadeIn>
          <FadeIn delay={0.1}>
            <motion.div className="bg-white rounded-2xl border border-[#E8ECF0] p-6 sm:p-10 shadow-sm max-w-5xl mx-auto" whileHover={{ boxShadow: "0 12px 50px rgba(28,29,98,0.08)" }}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {lenders.map((lender, idx) => (
                      <motion.div key={idx} className="bg-[#F0F4FF] rounded-xl border border-[#E8ECF0] p-4 text-center shadow-sm transition-all duration-300" whileHover={{ y: -3, boxShadow: "0 6px 20px rgba(28,29,98,0.1)", borderColor: service.color }} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 * idx }}>
                        ...
                        <span className="text-xs font-semibold text-[#1C1D62] block">{lender.type}</span>
              ...
              <div className="mt-8 pt-6 border-t border-[#E8ECF0] text-center">
                <p className="text-sm text-[#2D3748]">
                  <span className="font-semibold" style={{ color: service.color }}>Single Credit enquiry</span> — your application is structured once and presented to multiple lenders, so you get the best terms without multiple hits on your credit score.
                </p>
              </div>
            </motion.div>
          </FadeIn>
      ...
      {/* ─── 5. Fund Raising Process Timeline — Infographic ─── */}
      <ProcessInfographic
        steps={processTimeline.map((item) => ({
          icon: item.icon,
          label: item.title,
          desc: item.desc,
        }))}
        color={service.color}
        heading="Fund Raising Timeline"
        subtext="A clear, step-by-step journey from your first consultation to receiving funds in your account."
        compact
      />
