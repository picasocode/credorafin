/* ────────────────────────────────────────────
   BLOG DATA — default posts (seed source + client fallback)
   NOTE: categoryIcon is a STRING (lucide icon name) resolved via
   src/lib/icon-registry.ts. These defaults are seeded into the DB
   (BlogPost table) on first run; admins manage posts thereafter.
   ──────────────────────────────────────────── */

export interface BlogPost {
  id: string;
  category: string;
  categoryIcon: string; // lucide icon name, e.g. "ShieldCheck"
  title: string;
  excerpt: string;
  content: string[];
  author: string;
...
/** Static category list used as a fallback / for the filter UI. */
