  ...
  const handleBrochureSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/brochure", {
        ...
        body: JSON.stringify({
          email,
          // Pass the slug so the backend can find an uploaded PDF if one exists.
      ...
      toast({
        ...
        description: data.message || "Your download is starting.",
      });

      // Actually trigger the file download
      if (data.downloadUrl) {
        // Use a hidden iframe so the browser downloads the file without navigating away
