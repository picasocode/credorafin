...
#   3. bunx prisma generate          (regenerates the TS client — safe, no DB write)
#   4. bun run build                 (standalone output)
#   5. copy .next/static + public/ into .next/standalone/
#   6. fix filesystem permissions for nginx (www-data)
#   7. pm2 reload (zero-downtime)
#   8. health check + verify CSS/JS chunks load
#
...
#   sudo ./deploy-ui.sh         # if filesystem perms need root
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")"
...
# ── 6. Filesystem permissions for nginx (www-data) ───────────────────────────
step "Fixing filesystem permissions for nginx"
HOME_DIR="$(eval echo ~"${USER:-$(whoami)}")"
if [ -d "$HOME_DIR" ]; then
  chmod o+x "$HOME_DIR" 2>/dev/null && ok "traversal granted on ${HOME_DIR}" || warn "could not chmod ${HOME_DIR}"
fi
...
# ── 8. Health + static-asset verification ────────────────────────────────────
step "Health check"
sleep 2
